				...
				<div class="ibox float-e-margins">
					...
					<div class="ibox-content">
						...
						<div class="ibox">
							...
							<h3><strong style="color:red;">2、《控制器/方法》：意思是按照 控制器/方法 名来对应设置文件访问权限，即ControllerName/ActionName</strong></h3>
							...
							<table class="table table-bordered table-striped table-hover">
								...
								<tbody>
                                    ...
                                    <tr class="{$tr_class}">
                                    <td>{$v.id}</td>
                                    <td style='text-align:left;padding-left:<if condition="$v.leftpin neq 0">{$v.leftpin}px</if>' >{$topCat}{$v.lefthtml}{$v.title}</td>
                                    <td style="text-align:left;">{$v.name}</td>
                                    <td>
                                    	<if condition='$v[status] eq 1'>
                                    		<button type="button" class="btn btn-primary btn-sm" onclick="changestatus({$v.id},0);">显示</button>
                                    	<else/>
                                    		<button type="button" class="btn btn-danger btn-sm" onclick="changestatus({$v.id},1);">隐藏</button>
			   							</if>
			   						</td>
			   						<td>{$v.create_time}</td>
			   						<td class="has-warning"><input name="sort[{$v.id}]" value="{$v.sort}" class="form-control" style="width:50px;text-align:center"/></td>
			   						<td>
