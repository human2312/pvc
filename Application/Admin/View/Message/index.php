										...
										<td style="text-align: center"><input class="checkbox i-checks" type="checkbox" id="allid[]" value="{$l['id']}"></td>
          								...
          								<td>
          									<if condition='$l.is_show eq Y'>
          										<button type="button" class="btn btn-primary btn-sm" onclick="changestatus({$l.id},'N');">显示</button>
			 								<else/>
			        							<button type="button" class="btn btn-danger btn-sm" onclick="changestatus({$l.id},'Y');">隐藏</button>
			 								</if>
			 							</td>
			 							<td>
