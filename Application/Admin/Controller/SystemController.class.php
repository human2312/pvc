...
class SystemController extends AuthController
{
    public function index_show()
    {
    	if($_SESSION['a_group_id']==4)
    	{
    	    ...
    	    //总收益
    	    $amount=$amount_tb+$amount_pdd+$amount_jd;
    	    $this->assign('amount',$amount);
    	    //今日收益
    	    $amount_today=$amount_tb_today+$amount_pdd_today+$amount_jd_today;
    	    $this->assign('amount_today',$amount_today);
    	    //本月收益
    	    $amount_month=$amount_tb_month+$amount_pdd_month+$amount_jd_month;
    ...
    {
        // 实例化Model类
        $model = new \Common\Model\MuchangConfigModel();
        // 查询数据
        $data = $model->find();
        if($_POST) {
            // 组装数据
            $data = array(
                'toudan'          => I('POST.toudan'),
                'gonggao'          => I('POST.gonggao'),
                'shousong'          => I('POST.shousong'),
                'zhouqi'          => I('POST.zhouqi'),
                'zhitui'          => I('POST.zhitui'),
                'tuandui'          => I('POST.tuandui'),
                'kouliang1'          => I('POST.kouliang1'),
                'kouliang2'          => I('POST.kouliang2'),
                'kouliang3'          => I('POST.kouliang3'),
                'kouliang4'          => I('POST.kouliang4'),
            );
            ...
            layout(false);
            if ($res) {
                $this->success('修改成功',U('/System/muchang'));
            } else {
                $this->error('系统出错，请重试');
            }
        }else {
