...
class UserController extends AuthController
{
    public function index()
    {
        ...
        //二级推荐人手机
        if(trim(I('get.referrer_phone2'))) {
            $referrer_phone=trim(I('get.referrer_phone2'));
            $res_referrer=$User->where("phone='$referrer_phone'")->find();
            if($res_referrer['uid']) {
                $referrer_id=$res_referrer['uid'];
                $UserLevel = new \Common\Model\UserLevelModel();
                $sub_referrer_arr = $UserLevel->where("l2='$referrer_id' or l3='$referrer_id' or l4='$referrer_id' or l5='$referrer_id' or l6='$referrer_id' or l7='$referrer_id' or l8='$referrer_id' or l9='$referrer_id' ")->select();
                $level1 = null;
                if(count($sub_referrer_arr) > 0) {
                    foreach ($sub_referrer_arr as $k1 => $v1) {
                        if($k1 == 0) {
                            $level1 = "(uid = ".$v1["uid"];
                        }
                        if($k1 == (count($sub_referrer_arr) -1)) {
                            $level1 .= " or uid = ".$v1["uid"] .") ";
                        }
                        if($k1 > 0 && $k1 != (count($sub_referrer_arr) -1)) {
                            $level1 .= " or uid = ".$v1["uid"];
                        }
                    }
                }
    ...
    public function statistics()
    {
    	...
    	//返利总收益
    	$amount=$tb_amount+$jd_amount+$pdd_amount;
    	...
    	//本月返利总额
    	$amount_month=$UserBalanceRecord->where("status='2' and action in ('tbk','tbk_r','tbk_r2','tbk_rt','jd','jd_r','jd_r2','jd_rt','pdd','pdd_r','pdd_r2','pdd_rt') and date_format(pay_time,'%Y-%m')=date_format(now(),'%Y-%m')")->sum('money');
    	$amount_month/=100;
