...
class UserController extends AuthController
{
		...
		if(trim(I('post.token')))
		{
			...
			if($res_token['code']!=0)
			{
				...
				$msg2=array(
                    ...
                    'daydanshu'=>0,
                    'star'=>0,
                    'daishoujifen' => $msg['daishoujifen'],
                    'daishouyue' => $msg['daishouyue'],
                    'tiaoma' => $msg['tiaoma'],
				);



                $msg2["shareurl"] = "http://muchang.web.html48.com/#/register?auth_code=".$msg['auth_code'];

                //检查图片是否存在
                $path = 'Public/Upload/User/code/11.png';
                if (!file_exists($path)) {
                    $text = $msg2["shareurl"];
                    //引入类库
                ...
                $msg2["shareurlimg"] = $this->url.'/'.$path;
    ...
    public function editUser()
    {
        ...
        if($nickname != null) {
            $UserDetail->where("user_id = $uid")->save(array("nickname" => $nickname));
        }
        ...
        if($avatar != null) {
            ...
            if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $img, $result)){
                ...
                if (file_put_contents($new_file, base64_decode(str_replace($result[1], '', $img)))){
                    $UserDetail->where("user_id = $uid")->save(array("avatar" => '/'.$new_file));
                }
            }
        }

        if($nickname != null) {
            $UserDetail->where("user_id = $uid")->save(array("sex" => $sex));
        }

        if($phone != null) {
            $User->where("uid = $uid")->save(array("phone" => $phone));
        }

        if($password != null) {
            $User->where("uid = $uid")->save(array("password" => $User->encrypt($password)));
        }

        if($paypassword != null) {
            $User->where("uid = $uid")->save(array("paypassword" => $User->encrypt($paypassword)));
        }
        $res = array(
            'code' => 200,
            'msg' => 'success'
