...
class ConsigneeAddressModel extends Model
{
	 ...
	 */
	public function getList($user_id,$limit = 0)
	{
	    if(intval($limit) > 0) {
            $list=$this->where("user_id='$user_id'")->order('is_default desc,id desc')->limit($limit)->select();
        } else {
            $list=$this->where("user_id='$user_id'")->order('is_default desc,id desc')->select();
        }
		if($list!==false)
		{
			return $list;
