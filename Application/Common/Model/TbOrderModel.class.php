					...
					if($res_balance!==false and $res_record!==false and $res_record_free!==false and $res_order!==false) {
						...
						if($UserMsg['referrer_id'] and ($UserMsg['group_id']=='1' or $UserMsg['group_id']=='2'))
						{
							...
							if($referrer_money>0) {
								...
								$res_record_r=$UserBalanceRecord->addLog($referrer_id, $referrer_money, $all_money_r, 'tbk_r','2',$trade_id,'1');
							}else {
								$res_balance_r=true;
								$res_record_r=true;
							}
							...
							if($res_balance_r!==false and $res_record_r!==false ) {
								...
								if($referrerMsg['referrer_id']) {
									...
									if($referrer_money2>0) {
										...
										$res_record_r2=$UserBalanceRecord->addLog($referrer_id2, $referrer_money2, $all_money_r2, 'tbk_r2','2',$trade_id,'1');
									}else {
										$res_balance_r2=true;
										$res_record_r2=true;
									}
												...
												if($is_vip<2) {
													...
													if($referrer_tgid=='3' or $referrer_tgid=='4') {
														...
														if($referrer_money_team>0) {
															...
															$res_record_rt=$UserBalanceRecord->addLog($referrer_tid, $referrer_money_team, $all_money_rt, 'tbk_rt','2',$trade_id,'1');
		...
		if($msg and $msg['tk_status']!='13') {
			...
			if($res_exist) {
				...
				if($groupMsg and $UserMsg) {
					...
					//保存余额变动记录-预估
					$res_record=$UserBalanceRecordTmp->addLog($uid,$money_user,$action='tbk',$trade_id,$type='1',$msg['create_time']);
					if($res_record!==false) {
						//给直接推荐、间接推荐人返利
						//并且购买用户是普通会员
						if($UserMsg['referrer_id'] and ($UserMsg['group_id']=='1' or $UserMsg['group_id']=='2'))
						{
							...
							$referrer_money=round($referrer_money,2);
							//保存余额变动记录
							$res_record_r=$UserBalanceRecordTmp->addLog($referrer_id,$referrer_money,$action='tbk_r',$trade_id,$type='1',$msg['create_time']);
							if($res_record_r!==false) {
								...
								if($referrerMsg['referrer_id']) {
									...
									if($referrer_money2>0) {
										...
										$referrer_money2=round($referrer_money2,2);
										//保存余额变动记录
										$res_record_r2=$UserBalanceRecordTmp->addLog($referrer_id2,$referrer_money2,$action='tbk_r2',$trade_id,$type='1',$msg['create_time']);
									}else {
										$res_record_r2=true;
									}
											...
											for($i=3;$i<$team_num;$i++) {
												if($is_vip<2) {
													...
													if($referrer_tgid=='3' or $referrer_tgid=='4') {
														...
														if($referrer_money_team>0) {
															//保存余额变动记录
															$res_record_rt=$UserBalanceRecordTmp->addLog($referrer_tid,$referrer_money_team,$action='tbk_rt',$trade_id,$type='1',$msg['create_time']);
